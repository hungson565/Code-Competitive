...
// shared_ptr<TrieNode > make_shared< TrieNode>() {
...
class Trie {
  ...
  void insert(int e, vector<shared_ptr<TrieNode>>& vec, int& index) {
    ...
    for (int i = pFactors.size() - 1; i >= 0; i--) {
      ...
      if (!pCrawl->children.count(element)) {
        // auto t = make_shared< TrieNode>();
        // pCrawl->children[element] = t;// make_shared< TrieNode>();

        if (index< vec.size() && vec[index] != nullptr) {
          pCrawl->children[element] = vec[index];
          index++;
        } else {
          pCrawl->children[element] = make_shared< TrieNode>();
        }
      }
  ...
  void insert2(int e, vector<shared_ptr<TrieNode>>& vec, int index) {
    // cout << "insert e " << e << endl;
    auto pFactors = getFactorization(e);
    shared_ptr<TrieNode> pCrawl = root;
...
void solve() {
  ...
  vector<shared_ptr<TrieNode>> vec;
  ...
  // {
    ...
    // vector<shared_ptr<TrieNode>> vec(100001);
    // for (int i = 0; i < 100001; i++) {
    //   // auto t = new TrieNode();
    //   vec[i] = make_shared<TrieNode>();
    // }
    // int i = 0;
